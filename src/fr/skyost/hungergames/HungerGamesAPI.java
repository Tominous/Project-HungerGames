...
public class HungerGamesAPI {
	...
	public static final void giveReward(final Player player, final int value) {
		if(HungerGames.currentStep == Step.GAME || HungerGames.currentStep == Step.SECOND_COUNTDOWN) {
			final ItemStack reward = HungerGames.rewards.get(value);
			if(reward != null) {
				if(HungerGames.spectatorsManager.hasSpectator(player)) {
					HungerGames.players.get(player).addReward(reward);
				}
				else {
	...
	private static final World createWorld(final String name) {
		...
		HungerGames.multiverseHook.createWorld(name, world.getEnvironment(), world.getSeed(), world.getWorldType(), world.canGenerateStructures(), generator == null ? null : generator.getClass().getName());
		return HungerGames.multiverseHook.getWorld(name);
	}
	...
	/**
	 * Creates a kit and saves it.
	 * 
	 * @param name The kit's name.
	 * @param items The kit's items.
	 * @throws IOException If there is an error while saving the kit.
	 */
	
	public static final void createKit(final String name, final ItemStack... items) throws IOException {
		createKit(name, items, true);
	}
	
	/**
	 * Creates a kit and saves it if you want.
	 * 
	 * @param name The kit's name.
	 * @param items The kit's items.
	 * @param save If you want to save the kit.
	 * @throws IOException If there is an error while saving the kit.
	 */
	
	@SuppressWarnings("unchecked")
	public static final void createKit(final String name, final ItemStack[] items, final boolean save) throws IOException {
		if(save) {
			final JSONObject kit = new JSONObject();
			final JSONArray content = new JSONArray();
			for(final ItemStack item : items) {
				content.add(new JsonItemStack(item).toJsonObject(null));
			}
			kit.put("name", name);
			kit.put("content", content);
			final File kitsDirectory = new File(HungerGames.config.kitsDirectory);
			if(!kitsDirectory.exists()) {
				kitsDirectory.mkdir();
			}
			Utils.writeToFile(new File(kitsDirectory, ChatColor.stripColor(name).toLowerCase() + ".json"), kit.toJSONString());
		}
		final ItemStack icon = items[0];
		final ItemMeta meta = icon.getItemMeta();
		meta.setDisplayName(name);
		icon.setItemMeta(meta);
		if(HungerGames.config.kitsPermissions) {
			Bukkit.getPluginManager().addPermission(new Permission("hungergames.kits." + ChatColor.stripColor(name).toLowerCase()));
		}
		HungerGames.kits.put(name, items);
		final List<ItemStack> icons = new ArrayList<ItemStack>();
		for(final ItemStack[] content : HungerGames.kits.values()) {
			icons.add(content[0]);
		}
		HungerGames.kitsMenu = Bukkit.createInventory(null, HungerGames.kitsMenu == null ? 9 : Utils.round(HungerGames.kits.size(), 9), HungerGames.config.kitsSelectorName);
		HungerGames.kitsMenu.addItem(icons.toArray(new ItemStack[icons.size()]));
	}
	...
	public static final void deleteKit(final String name) {
		final File kitsDirectory = new File(HungerGames.config.kitsDirectory);
		if(!kitsDirectory.exists()) {
			kitsDirectory.mkdir();
			return;
		}
		final ItemStack[] content = HungerGames.kits.remove(name);
		HungerGames.kitsMenu.remove(content[0]);
		final File kitFile = new File(kitsDirectory, ChatColor.stripColor(name).toLowerCase());
		if(kitFile.exists()) {
			kitFile.delete();
		}
		else {
			HungerGames.logsManager.log("The file \"" + kitFile.getPath() + "\" was not found. The kit " + name + " was not deleted.", Level.WARNING);
		}
	}
